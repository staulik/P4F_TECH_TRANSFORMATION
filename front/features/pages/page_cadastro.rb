class ScreenCadastro < SitePrism::Page
 ...
 def cadastrar_usuario
  ...
  def salvar_usuario
   path = File.expand_path("../../support/massa/users.yml", __FILE__)
   read_path = YAML::load_file(File.open(path))
   pp read_path[:cad_user][:user] = input_email_create.value  #Modify
   #File.open((read_path), 'w') {|f| f.write read_path.to_yml } # 
  end
